...
extension UTXOTransactionHistoryMapper: TransactionHistoryMapper {
    func mapToTransactionRecords(
        ...
        return transactions.compactMap { transaction -> TransactionRecord? in
            mapToTransactionRecord(transaction: transaction, address: walletAddress)
        }
    }

    func mapToTransactionRecord(transaction: BlockBookAddressResponse.Transaction, address: String) -> TransactionRecord? {
        guard let feeSatoshi = Decimal(stringValue: transaction.fees) else {
            return nil
        }

        let isOutgoing = transaction.compat.vin.contains(where: { $0.addresses.contains(address) })
...
private extension UTXOTransactionHistoryMapper {
