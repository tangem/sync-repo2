...
//            self.expand()
    ...
    private func inset(for state: BottomScrollableSheetState) -> CGFloat {
        switch state {
        case .bottom:
            return sheetContainerHeight - headerHeight - 10.0 // TODO: Andrey Fedorov - Find out where it comes from
        case .top:
            return diff // FIXME: Andrey Fedorov - Test only, remove when not needed
        }
    }
